...
public class Web3Utils {
    ...
    private static final String CONTRACT_ADDRESS = "0xCc4439566EC030A30ab86B2A46486FF7b769CbDc";
    ...
    public static String indexHash(String hash) throws Exception {
        ...
        TransactionReceipt result = contract.indexDigest(Numeric.hexStringToByteArray(hash)).send();
    ...
    public static boolean verifyAssertion(String token, String url) throws Exception {
            ...
            VerificationContract contract = VerificationContract.load(CONTRACT_ADDRESS, web3, credentials, GAS_PRICE, VerificationContract.GAS_LIMIT);
            TransactionReceipt result = contract.verifyAssertion(Numeric.hexStringToByteArray(token), url, BigInteger.ZERO).send();
            String id = result.getLogs().get(2).getData();
            int answer = 0;
            int attempt = 0;
            while (answer == 0 && attempt++<10) {
                TimeUnit.SECONDS.sleep(5);
                result = contract.checkResult(Numeric.hexStringToByteArray(id)).send();
